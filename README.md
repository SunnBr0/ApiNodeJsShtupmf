# API для обработки обращений

Этот проект представляет собой backend-сервис для регистрации и обработки обращений. API позволяет создавать обращения, получать их список и отдельные записи по `id`, фильтровать обращения по дате или диапазону дат, переводить их в работу, завершать, отменять и массово отменять обращения со статусом `В работе`.

Проект написан на `Node.js`, `Express` и `TypeScript`, для работы с базой данных используются `Prisma` и `PostgreSQL`. Структура приложения разделена на слои `domain`, `presentation`, `infrastructure` и `main`: бизнес-правила вынесены в use case, HTTP-слой отделен от инфраструктуры, а доступ к данным реализован через интерфейс репозитория.
